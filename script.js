...
const initLightbox = () => {
  const images = Array.from(
    document.querySelectorAll(".zoomable, .img-frame img, .state-card img"),
  );
  ...
  const updateZoomUI = () => {
    ...
    const fillPercent = ((currentScale-1) / 29) * 100;
  ...
  lightbox.addEventListener(
    ...
    (e) => {
      ...
      currentScale = Math.min(Math.max(1, currentScale + delta), 30);
